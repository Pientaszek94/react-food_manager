...
DATABASE_URL=os.environ.get('DATABASE_URL') or 'mongodb+srv://pawel123:aplikacja123@cluster0.k46cr.mongodb.net/'
print("DATABSE_URL:\n",DATABASE_URL)

client = MongoClient(DATABASE_URL)
db = client['foodmanager']
...
class User:
    ...
    """MY UPDATE ARRAYS"""
    def update_recipes_push(self,user_id, data):
        user=db.users.update_one(
            {"_id": bson.ObjectId(user_id)},
            {
            "$push": data
    ...
    def update_recipes_pull(self,user_id, data):
        user=db.users.update_one(
            {"_id": bson.ObjectId(user_id)},
            {
            "$pull": data
